...
public class CourseParamRepositoryImpl implements CourseParamRepository {
	 ...
	 */
	@Override
	@Caching(cacheable = @Cacheable(key = "#courseParam.instructor"), put = { @CachePut(key = "#result.id") })
	public CourseParam insertOne(CourseParam courseParam) {
		courseParamMapper.insert(courseParam);
		return courseParam;
	}
